...
void GridMapRosConverter::toGetMapResponse(const grid_map::GridMap& gridMap, const std::string& layer,
                            float dataMin, float dataMax, nav_msgs::GetMap::Response& response)
{
  response.map.header.frame_id = gridMap.getFrameId();
  response.map.header.stamp.fromNSec(gridMap.getTimestamp());
  response.map.info.resolution = gridMap.getResolution();
  response.map.info.width = gridMap.getSize()(0);
  response.map.info.height = gridMap.getSize()(1);
  Position position = gridMap.getPosition() - 0.5 * gridMap.getLength().matrix();
  response.map.info.origin.position.x = position.x();
  response.map.info.origin.position.y = position.y();
  ...
  response.map.data.resize(nCells);
  ...
  for (GridMapIterator iterator(gridMap); !iterator.isPastEnd(); ++iterator) {
    ...
    float value = (dataMax - gridMap.at(layer, *iterator)) / (dataMax - dataMin);
    if (isnan(value))
      value = -1;
    else
    ...
    response.map.data[nCells - index - 1] = value;
  }
}
                                          ...
                                          nav_msgs::OccupancyGrid& occupancyGrid)
{
  ...
  for (GridMapIterator iterator(gridMap); !iterator.isPastEnd(); ++iterator) {
    ...
    if (isnan(value))
      value = -1;
    // add threshold for obstacles, currently hard coded
    else if(value > 0.25)
      value = cellMax;
    else
      value = cellMin + min(max(0.0f, value), 1.0f) * cellRange;
